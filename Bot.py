...
async def on_member_join(member):
    ...
    try:
        message = await channel.send(content="???????????????? ???????? ?????? ????????. ???? ?????????? ?????? ??????, ???????? ?????????????????????? ???? ????????????.\n\n__Welcome Summoner <@%d> to 𝕊𝕠𝕦𝕝 ℍ𝕒𝕔𝕜𝕖𝕣𝕤 𝟚?__ \n__Please make sure to read <#953856487330050059> and enjoy your stay?__"%member.id,file=discord.File('output.png'), delete_after = 360)
    except:
